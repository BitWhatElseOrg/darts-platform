...
Das Production-Projekt `dartbase` liegt im Railway-Workspace
`BitWhatElse Projects`. Im Environment `production` sind Web, API,
Statistik-Worker, PostgreSQL und Redis erfolgreich deployt. `dartbase.ch` und
`api.dartbase.ch` sind öffentlich erreichbar; DNS-Verifikation, Zertifikate und
die Web-/API-Smoke-Tests sind erfolgreich.
...
Auth-Konfigurationswerte wie die API. Die Railway-IaC bildet das angelegte
Projekt einschließlich Worker, Domains, Volumes, Build-/Start-Kommandos und
bestehender Variablen vollständig ab. Ein `railway config plan` meldet für
Production keine Änderungen.
