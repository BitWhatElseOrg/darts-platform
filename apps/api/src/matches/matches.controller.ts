...
import { boardControllerLeaseRequestSchema, createMatchSchema, submitVisitSchema, undoVisitSchema, type BoardControllerLeaseRequest, type BoardControllerLeaseResponse, type CreateMatchInput, type MatchStateResponse, type SubmitVisitInput, type UndoVisitInput } from "@darts-platform/schemas";
...
export class MatchesController {
  ...
  @Post(":matchId/controller-lease") public controllerLease(@Param("organizationId", ParseUUIDPipe) organizationId: string, @Param("matchId", ParseUUIDPipe) matchId: string, @Body() body: unknown, @CurrentAuth() auth: AuthContext, @Req() request: FastifyRequest): Promise<BoardControllerLeaseResponse> {
    ...
    return this.service.acquireControllerLease({ organizationId, matchId, controllerId: data.controllerId, force: data.force, auth, audit: getAuditContext(request) });
  }
}
