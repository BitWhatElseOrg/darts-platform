...
function LiveBoard({ board, mode, tournamentId }: { readonly board: BoardSlot; readonly mode: LiveTournamentProps["mode"]; readonly tournamentId: string }) {
  ...
  return <article className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
    ...
    {board.match === null ? <p className="mt-8 text-xl text-slate-500">Kein aktives Match</p> : <div className="mt-5 grid grid-cols-2 gap-3">{board.match.participants.map((participant) => <div className={participant.isActive ? "rounded-xl bg-emerald-400/10 p-3" : "p-3"} key={participant.playerId}><p className="truncate text-sm">{participant.displayName}</p><p className="mt-2 text-5xl font-black tabular-nums">{participant.remaining}</p><p className="mt-1 text-sm text-slate-400">{participant.legsWon} Legs</p></div>)}</div>}
  </article>;
}
