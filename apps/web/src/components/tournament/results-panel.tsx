  ...
  return (
    <section aria-labelledby="results-heading">
      <div className="flex items-baseline justify-between gap-3 pb-2">
        ...
        <span className="shrink-0 font-numerals text-counter font-bold tabular text-sisal-500">{results.length}</span>
      </div>
      <Rule />
      {results.length === 0 ? (
        <p className="py-4 font-plate text-body text-sisal-500">Noch kein Ergebnis erfasst.</p>
      ) : (
        <ul>
          {results.map((result) => (
            <li className="border-b border-sisal-300 py-3" key={result.matchId}>
              <p className="font-plate text-caption font-semibold uppercase tracking-[0.12em] text-sisal-500">{result.stageLabel}</p>
              <p className="mt-1 font-plate text-body font-semibold text-wedge-900">
              ...
              {canCorrect && result.resultType === "PLAYED" && selectedId === result.matchId ? (
                <div className="mt-3 flex flex-col gap-2">
                  <label className="font-plate text-caption font-semibold uppercase tracking-[0.12em] text-sisal-500" htmlFor={`correction-${result.matchId}`}>Korrekturgrund</label>
                  <textarea
                  ...
                  <p className="font-plate text-caption text-sisal-500">Der letzte entscheidende Visit wird auditiert zurückgenommen. Gestartete Folgematches blockieren die Korrektur.</p>
                </div>
              ) : null}
            </li>
          ))}
