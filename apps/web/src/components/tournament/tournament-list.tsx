      ...
      <div className="mx-auto max-w-[1100px] px-5 py-8 xl:px-9">
        ...
        {organizationsQuery.data && organizationsQuery.data.length > 1 ? (
          <div className="mt-5 max-w-sm">
            <label className="font-plate text-[0.75rem] font-semibold uppercase tracking-[0.14em] text-sisal-500" htmlFor="tournament-organization">Organisation</label>
                ...
                <li className="border-b border-sisal-300" key={tournament.id}>
                  <Link className="group flex flex-wrap items-center gap-x-6 gap-y-3 py-4 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring-green" href={`/turniere/${tournament.id}?organisation=${organization.id}`}>
                    <div className="min-w-0 flex-1">
                      <h2 className="font-numerals text-[1.5rem] leading-tight font-bold tracking-[-0.01em] text-wedge-900 group-hover:underline group-hover:decoration-1 group-hover:underline-offset-4">{tournament.name}</h2>
                      <p className="mt-0.5 font-plate text-[0.875rem] text-sisal-500">{calendarDate(tournament.startsAt)} · {tournament.participantCount} Teilnehmer · {tournament.boardCount} Boards</p>
                    </div>
                    <div className="w-28">
...
function Notice({ children }: { readonly children: ReactNode }) {
  return <div className="mt-6 border border-sisal-400 bg-sisal-100 px-6 py-10 text-center font-plate text-[0.875rem] text-wedge-900">{children}</div>;
}
